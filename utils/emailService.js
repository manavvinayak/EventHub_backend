...
const getEmailHTML = (userName, eventDetails) => {
  return `
...
const getEmailText = (userName, eventDetails) => {
  return `Hey ${userName}!
...
export const sendRegistrationConfirmationEmail = async (userEmail, userName, eventDetails) => {
  try {
    ...
    if (process.env.MAILGUN_API_KEY && process.env.MAILGUN_DOMAIN) {
      ...
      const fromEmail = process.env.SENDGRID_FROM_EMAIL || 'noreply@eventhub.com'
      ...
      console.log('✅ Registration confirmation email sent via SendGrid:', result[0].statusCode)
      return { success: true, messageId: result[0].headers['x-message-id'] }
      
    } else {
      console.log('📧 Using SMTP (Gmail) for email delivery')
      ...
      console.log('✅ Registration confirmation email sent via SMTP:', info.messageId)
...
export const sendNotificationEmail = async (userEmail, userName, subject, message) => {
  try {
    if (process.env.SENDGRID_API_KEY) {
      ...
      const msg = {
        to: userEmail,
        from: process.env.SENDGRID_FROM_EMAIL || 'noreply@eventhub.com',
        subject: subject,
        html: `
